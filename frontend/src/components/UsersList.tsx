    ...
    <div className="min-h-screen bg-[#F4EBD3]">
      <header className="bg-[#DED3C4] shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-3xl font-bold text-[#555879]">Liste des utilisateurs</h1>
          <div>
            <button
              onClick={() => navigate('/add-user')}
              className="bg-[#98A1BC] hover:bg-[#98A1BC]/90 text-[#F4EBD3] font-bold py-2 px-4 rounded mr-2"
            >
              Ajouter utilisateur
            </button>
            <button
        ...
        <div className="px-4 py-6 sm:px-0">
                  ...
                  <div className="px-6 py-4 sm:px-8">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-[#555879]">{user.nomcomplet}</div>
                          <div className="text-sm text-[#98A1BC]">{user.email}</div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                          user.role === 'super-admin'
                            ...
                            : user.role === 'admin'
                            ? 'bg-[#98A1BC] text-[#F4EBD3]'
                            : 'bg-[#DED3C4] text-[#555879]'
                        }`}>
                          {user.role}
                        </span>
                        {user.entrepriseId && (
                          <span className="ml-3 text-sm text-[#98A1BC]">
          ...
          {users.length === 0 && (
            <div className="text-center py-12">
              <p className="text-[#98A1BC]">Aucun utilisateur trouvé.</p>
            </div>
          )}
        </div>
      </main>
